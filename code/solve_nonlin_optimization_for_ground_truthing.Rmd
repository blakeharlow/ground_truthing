...
good_sightings <- good_data %>% makeCleanData()
  
# This is a vector showing the order of the <x> vector. This should
# only have positions that are included in the sightins of clean_data
...
compute_residuals <- function(X, sightings_tbl, y_measured){
  ...
  return(F_of_X(X, sightings_tbl) - y_measured)
}
...
compute_residuals(X_guess_trunc_scaled, clean_data_scaled)[1]
...
fit <- optim(X_guess_trunc_scaled, function_to_minimize, 
             data_tbl = clean_data_scaled, y_measured = y_measured,
             method = "BFGS", control = list(trace = TRUE))
```

```{r}
# Convert a vector back to meters and undo origin shift.
# Also add first point back in:

undo_shift_and_scale <- function(input) {
  output <- input / len_scale
  output <- c(0, 0, 0, output)
  output <- output + X_guess_o_shift
  return(output)
}
```
...
```{r}
X_solution <- undo_shift_and_scale(fit$par)
...
X_guess_tbl <- tibble(position = levels, 
                    x = X_guess[c(TRUE, FALSE, FALSE)],
                    y = X_guess[c(FALSE, TRUE, FALSE)],
                    z = X_guess[c(FALSE, FALSE, TRUE)])
...
# Now compute residuals and add them to clean_data
residuals <- compute_residuals(fit$par, clean_data_scaled) %>% matrix(ncol = 3, byrow = TRUE)
colnames(residuals) <- c("x_residuals","y_residuals","z_residuals")
residuals <- residuals %>% as_tibble() %>% 
  mutate(residuals = (x_residuals^2 + y_residuals^2 + z_residuals^2)^(1/2))

residuals <- F_of_x(fit$par, clean_data_scaled) - y_measured %>% 
...
# ggplot() +
#   geom_point(data = X_guess_tbl, 
#              aes(x,y)) +
#   geom_point(data = final_tbl,
#              aes(x,y), color = "red")


ggplot() +
  geom_point(data = X_solution_tbl, aes(x,y)) +
  geom_text(data = X_solution_tbl, aes(x, y, label = position),
            hjust=-0.5, vjust=0, size = 3) +
  geom_segment(data = clean_data,
              aes(x = x_start, y = y_start, xend = x_end, yend = y_end), color = "gray")
```